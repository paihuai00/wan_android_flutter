...
import 'package:wan_android_flutter/dios/default_http_transformer.dart';
...
BaseDioResponse handleResponse(Response? response,
    {HttpDioTransformer? httpTransformer}) {
  httpTransformer ??= DefaultHttpTransformer.getInstance();

// 返回值异常
  if (response == null) {
    return BaseDioResponse.failureFromError();
  }
...
BaseDioResponse handleException(Exception exception) {
  var parseException = _parseException(exception);
  return BaseDioResponse.failureFromError(parseException);
}
...
HttpDioException _parseException(Exception error) {
